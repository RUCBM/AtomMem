...
This is your recent action history:
<history>
{history}
</history>
...
Example 1:
Based on current memory, I should first open the door of art studio, so I need to <action>open art studio door</action><update_query>The action history in the art studio.</update_query>
Example 2:
<add_memory>The orange was placed in my inventory.</add_memory>. I need to go to the garden. <action>go to the garden</action>
Example 3:
<modify_memory>The orange was no longer in my inventory, I ate it.</modify_memory> <action>look around</action>
"""
...
class SCIMemAgent(RAgent):
    ...
    def start(self, gen_batch: DataProto, timing_raw: dict):
        ...
        self.envs: List[ScienceWorldEnv]= [ScienceWorldEnv("") for i in range(self.bsz)]
        for idx in range(self.bsz):
            ...
            self.envs[idx].load(env_name, var)
            self.prompt[idx] = self.envs[idx].get_task_description()
    ...
    def update(self, gen_output: DataProto) -> DataProto:
        ...
        if not torch.all(self.is_final):
            ...
            for idx, real_idx in enumerate(true_indices):
                ...
                if is_valid:
                    if action:
                        observation, reward, isCompleted, info = self.envs[real_idx].step(action)
                        self.observations[real_idx] = observation
                        self.history[real_idx] += '\n'+ observation
                        self.reward[real_idx] = reward
                        self.is_final[real_idx] = isCompleted
                else:
