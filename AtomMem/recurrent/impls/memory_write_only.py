...
TEMPLATE = """You are presented with a problem, a section of an article that may contain the answer to the problem, and a previous memory. Please read the provided section carefully and take two action based on the section: 1) retrieve 2) do nothong 3) update memory. Be sure to retain all relevant details which may be useful in the future.
...
class MemoryAgent(RAgent):
    ...
    def action(self) -> Tuple[List[torch.Tensor], dict]:
        ...
        if active_mask.sum().item() == 0:
            ...
            for idx in range(self.bsz):
                retriever = self.vectorstore.as_retriever(search_kwargs={"k": 5, "filter": {"trajectory_id": idx}, "fetch_k": 5000})
                prompt = gen_batch.non_tensor_batch['prompt_ids']
                prompt_str = prompt[idx]
                prompt_str = self.tokenizer.decode(prompt_str, skip_special_tokens=True)
                memory = retriever.invoke(prompt_str)
                memory_text = self.tokenizer.encode("\n".join(doc.page_content for doc in memory)[:self.config.max_memorization_length], add_special_tokens=False) if memory else self.NO_MEMORY_TOKENS
                message = self.token_final_message_template.format(
                                prompt=prompt[idx],
                                memory=memory_text, # use pre-tokenized "No previous memory" for first round
                        )
                # print('-------------------')
                # print(re.search(r'<memory>\s*(.*?)\s*</memory>', self.tokenizer.decode(message), re.DOTALL).group(1).strip())
            ...
            prompt = gen_batch.non_tensor_batch['prompt_ids']
            chunk = gen_batch.batch['context_ids'][:, self.config.chunk_size * self.step-100: self.config.chunk_size * (self.step+1)+100] # bs * chunk_size
            ...
            for idx, val in enumerate(active_mask):
                if val:
                    ...
                    prompt_str = prompt[idx]
                    prompt_str = self.tokenizer.decode(prompt_str, skip_special_tokens=True)
                    chunk_str = self.tokenizer.decode(chunk[idx][chunk[idx] != self.tokenizer.pad_token_id], skip_special_tokens=True)
                    memory = retriever.invoke(prompt_str + chunk_str)
                    memory_text = self.tokenizer.encode("\n".join(doc.page_content for doc in memory)[:self.config.max_memorization_length], add_special_tokens=False) if memory else self.NO_MEMORY_TOKENS
                    message = self.token_message_template.format(
                                prompt=prompt[idx],
                                memory=memory_text, # use pre-tokenized "No previous memory" for first round
                                chunk=chunk[idx][chunk[idx] != self.tokenizer.pad_token_id], # unpadding needed here
                        )
                    # print('-------------------')
                    # print(re.search(r'<memory>\s*(.*?)\s*</memory>', self.tokenizer.decode(message), re.DOTALL).group(1).strip())
                    self.messages.append(message)
    ...
    def update(self, gen_output: DataProto) -> DataProto:
        action_rate_metric = {"action_rate/do_nothing":0,"action_rate/not_follow": 0, "total": gen_output.batch['responses'].shape[0]}
        if not self.is_final:
            ...
            for idx, real_idx in enumerate(true_indices):
                ...
                if "update memory: " in text:
                    start_index = text.find('update memory: ')
                    content = text[start_index + len('update memory: '):].strip()
                    docs = [Document(page_content=content, metadata = {"trajectory_id": real_idx})]
                    self.vectorstore.add_documents(docs)
                elif "do nothing" in text:
