    ...
    async def tool_call_async(self, texts, true_indices, action_rate_metric):
        async def async_wrapper(idx, real_idx):
            ...
            if tool_message:
                ...
                try:
                    json_tool_call = json.loads(tool_call_text)
                    tool_name = json_tool_call["name"]
                except:
                    tool_name = "failed"
                if tool_name == "search" or tool_name == "failed":
                    # 如果调用失败，则直接放到action中；如果 tool_name为search，则直接放入recent action中
                    self.recent_action[real_idx] = recent_round
                elif tool_name == "fetch_url":
                    # 如果 tool_name为fetch_url，则过browser agent然后放入recent action中
                    purpose = json_tool_call["arguments"]["purpose"]
                    context = json.dumps(tool_content,ensure_ascii=False)
                    browser_agent_prompt = BROWSER_AGENT_PROMPT.format(raw_content=context, context_awareness_prompt=purpose)
