...
TEMPLATE = """You are presented with a section of an article that may contain the answer to the problem, and a previous memory. Please read the provided section carefully and select an action based on the section: 1) retrieve 2) do nothong 3) update memory. 
retrieve is used to generate a query to retrieve potentially relevant entries from the memory bank and populate the memory section.
...
<memory>
You have retrieved {retrievetimes} times, the max time for retrieve is 1.
{memory}
</memory>
...
Example 1:
retrieve: The dance partner of Yulia.
Example 2:
do nothing
Example 3:
...
TEMPLATE_NO_RETRIEVE = """You are presented with a problem, a section of an article that may contain the answer to the problem, and a previous memory. Please read the provided section carefully and select an action based on the section: 1) do nothong 2) update memory. Be sure to retain all relevant details which may be useful in the future.
...
TEMPLATE_FINAL_BOXED = """You are presented with a problem and a previous memory. You can use 'retrieve' to generate a query to retrieve potentially relevant entries from the memory bank and populate the memory section or use 'final answer' to answer the problem in \\boxed{{}}.
...
Example 1:
retrieve: Who is the dance partner of Yulia?
Example 2:
final answer: ..., therefore the answer is \\boxed{{}}.
"""

TEMPLATE_FINAL_BOXED_NO_RETRIEVE = """You are presented with a problem and a previous memory. You can use 'final answer' to answer the problem in \\boxed{{}}.
...
class MemoryAgent(RAgent):
    ...
    def start(self, gen_batch: DataProto, timing_raw: dict):
        ...
        self.retrieve_times = [0]*self.bsz
    ...
    def action(self) -> Tuple[List[torch.Tensor], dict]:
        ...
        for idx in active_indices:
            ...
            if not context_mask[idx]:
                full_memory_text = "\n".join(doc.page_content for doc in self.temp_memory[idx]) if self.temp_memory[idx] else self.NO_MEMORY_TOKENS
                token_memory_text = self.tokenizer.encode(full_memory_text, add_special_tokens=False)
                if len(token_memory_text) > self.config.max_memorization_length:
                    # cut memory_text by max_memorization_length
                    memory_text = self.tokenizer.decode(token_memory_text[:self.config.max_memorization_length])
                else:
                    memory_text = full_memory_text
                if self.retrieve_times[idx] < 1:
                    message = TEMPLATE_FINAL_BOXED.format(
                                    retrievetimes = str(self.retrieve_times[idx]),
                                    memory=memory_text)
                else:
                    message = TEMPLATE_FINAL_BOXED_NO_RETRIEVE.format(memory=memory_text)
            ...
            # 如果仍有context，则使用常规prompt
                ...
                # cut memory_text by max_memorization_length
                    ...
                    memory_text = full_memory_text
                if self.retrieve_times[idx] < 1:
                    # 如果超出最大检索次数，强制不使用retrieve
                    message = TEMPLATE.format(
                                retrievetimes = self.retrieve_times[idx],
                                memory=memory_text,
                    ...
                    message = TEMPLATE_NO_RETRIEVE.format(
                                memory=memory_text, # use pre-tokenized "No previous memory" for first round
                                chunk=self.tokenizer.decode(chunk[idx][chunk[idx] != self.tokenizer.pad_token_id],skip_special_tokens=True), # unpadding needed here
                        )
                self.context_sliding_windows[idx].append(self.tokenizer, "user", message)
                tensor_messages = self.context_sliding_windows[idx].build(self.tokenizer)
    ...
    def update(self, gen_output: DataProto) -> DataProto:
        action_rate_metric = {"action_rate/do_nothing":0,"action_rate/not_follow": 0, "action_rate/retrieve": 0, "total": gen_output.batch['responses'].shape[0]}
        if not torch.all(self.is_final):
            ...
            for idx, real_idx in enumerate(true_indices):
                ...
                if "update memory: " in text and "do nothing" not in text and "retrieve" not in text:
                    ...
                    self.retrieve_times[real_idx] = 0
                    self.context_step[real_idx] += 1
                elif "do nothing" in text and "update memory: " not in text and "retrieve" not in text:
                    self.temp_memory[real_idx] = []
                    action_rate_metric['action_rate/do_nothing'] += 1
                    self.retrieve_times[real_idx] = 0
                    self.context_step[real_idx] += 1
                elif "retrieve" in text and "update memory: " not in text and "do nothing" not in text:
                    if self.retrieve_times[real_idx]>1:
                        # 如果仍然超出最大retrieve次数，报错warning并当作do nothing处理
                        warnings.warn(f"Sample {real_idx} has been retrieved {int(self.retrieve_times[real_idx])} times (>1).")
                        self.retrieve_times[real_idx] = 0
                        self.context_step[real_idx] += 1
                        continue
                    query = text.removeprefix("retrieve: ")
                    retriever = self.vectorstore.as_retriever(search_kwargs={"k": 2, "filter": {"trajectory_id": real_idx},"fetch_k": 5000})
                    memory = retriever.invoke(query)
                    self.union_memory(real_idx, memory)
                    action_rate_metric['action_rate/retrieve'] += 1
                    self.retrieve_times[real_idx] += 1
    ...
    def union_memory(
        ...
        """
        将 new_docs 合并到 self.temp_memory
        """
        # 建立 dict：key=doc_id, value=Document
        memory_map: dict[str, Document] = {
            d.metadata.get("doc_id", str(uuid4())): d for d in self.temp_memory[idx]
        }

        # 处理新文档
        for doc in new_docs:
            doc_id = doc.metadata.get("doc_id", str(uuid4()))
            memory_map[doc_id] = doc        # 覆盖或新增

        # 按插入顺序（新在前）返回
        self.temp_memory[idx] = list(memory_map.values())
        return
