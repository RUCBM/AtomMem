...
class MemoryAgent(RAgent):
    ...
    def action(self) -> Tuple[List[torch.Tensor], dict]:
        ...
        for idx in active_indices:
            ...
            if not context_mask[idx]:
                ...
                memory_text = ""
                retriever = self.vectorstore.as_retriever(search_kwargs={"k": 12, "filter": {"trajectory_id": idx},"fetch_k": 16000})
                ...
                message = [{"role": "system", "content": SYSTEM_PROMPT}, 
                           {"role": "user", "content": TEMPLATE_FINAL_BOXED.format(
                            ...
                            short_memory = self.short_memory[idx],
            ...
            # 如果仍有context，则使用常规prompt
                ...
                # get memory
                memory_text = ""
                retriever = self.vectorstore.as_retriever(search_kwargs={"k": 12, "filter": {"trajectory_id": idx},"fetch_k": 40000})
                memory = retriever.invoke(self.query[idx])
                self.temp_memory[idx] = memory
                memory_text += "\n".join(f"Memory {i+1}: {doc.page_content}" for i, doc in enumerate(memory)) if memory else self.NO_MEMORY_TOKENS
                del retriever
    ...
    def update(self, gen_output: DataProto) -> DataProto:
        ...
        if not torch.all(self.is_final):
            ...
            prompts = unpad(self.tokenizer, gen_output.batch['prompts'], remove_eos=True)
            for idx, real_idx in enumerate(true_indices):
                prompt = self.tokenizer.decode(prompts[idx], skip_special_tokens=True)
                documents = self.extract_documents(prompt)
                self.add_all_document(real_idx, documents, action_rate_metric)
    ...
    def add_all_document(self, idx, documents, action_rate_metric):
        for document in documents:
            self._handle_add(idx, document, action_rate_metric)
    ...
    def extract_documents(self, text: str) -> List[str]:
        """
        Extract content between 'This is the article:' and 'Important:',
        then split by 'Document i:' markers and return a list of document strings.
        """

        # 1. 提取指定区间
        pattern = r"This is the article:(.*?)Important:"
        ...
        # 2. 按 Document i: 分割
        # 会得到 ['', doc1, doc2, ...] 或 [doc1, doc2, ...]
        docs = re.split(r"Document\s+\d+:", article_block)
