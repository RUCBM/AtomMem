...
class MemoryAgent(RAgent):
    ...
    def action(self) -> Tuple[List[torch.Tensor], dict]:
        ...
        for idx in active_indices:
            ...
            if not context_mask[idx]:
                ...
                self.trajectory_logs[idx]["steps"].append({
                    "step": self.agent_step[idx],
                    "prompt": message,
                    "completion": None
                })
    ...
    def update(self, gen_output: DataProto) -> DataProto:
        ...
        if not torch.all(self.is_final):
            ...
            for idx, real_idx in enumerate(true_indices):
                ...
                # 找到刚才那一步
                self.trajectory_logs[real_idx]["steps"][-1]["completion"] = text
    ...
    def dump_trajectory_logs(
        self,
        folder="/nfsdata/huoyupeng/Memory_Agent/eval/memory_agent/trajectory_logs"
    ):
        import os
        import json
        ...
        with open(path, "w", encoding="utf-8") as f:
            json.dump(self.trajectory_logs, f, indent=2, ensure_ascii=False)
