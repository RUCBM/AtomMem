...
class MockBatch:
    def __init__(self, instruction):
        ...
        self.non_tensor_batch = {"index": ["train_1", "train_2", "train_3"]}

class MockDataProto:
    def __init__(self, instruction):
        self.batch = {"instruction": instruction, "responses": torch.randint(0, 100, instruction.shape)}
        self.non_tensor_batch = {"index": ["train_1", "train_2", "train_3"]}
        self.meta_info = {}
...
def test_memory_agent_4round(device):
    ...
    actions = [
        [
            "<add_memory>round-1-memory</add_memory><update_query>round-1-query</update_query><web>search[hello]</web>",
            "<update_query>round-2-query</update_query><web>search[hello]</web>",
            "<do_nothing></do_nothing><web>search[hello]</web>"
        ],
        [
            "<modify_memory>Document 1: round-1-memory-modified</modify_memory><web>click[Next >]</web>",
            "<add_memory>round-2-memory</add_memory><web>click[Next >]</web>",
            "<web>click[Next >]</web>"
        ]
    ]
