...
TEMPLATE_FINAL_BOXED = """You are presented with a problem and a previous memory. Please answer the problem based on the previous memory and put the answer in \\boxed{{}}. Output the answer only as an exact match—do not add any description.
...
class MemoryAgent(RAgent):
    ...
    def action(self) -> Tuple[List[torch.Tensor], dict]:
        ...
        for idx in active_indices:
            ...
            if not context_mask[idx]:
                ...
                # build message
                message = [{"role": "user", "content": TEMPLATE_FINAL_BOXED.format(prompt = self.prompt[idx][self.answer_idx[idx]], memory=self.memory[idx])}]
            ...
            # 如果仍有context，则使用常规prompt
                ...
                message = [{"role": "user", "content": TEMPLATE.format(
                            ...
                            chunk=self.tokenizer.decode(chunk[idx][chunk[idx] != self.tokenizer.pad_token_id],skip_special_tokens=True),
                            )}]
                message = self.tokenizer.apply_chat_template(message, tokenize=True, add_generation_prompt=True, return_tensors="pt")[0]
                self.messages.append(message)
    ...
    def update(self, gen_output: DataProto) -> DataProto:
        if not torch.all(self.is_final):
            ...
            for idx, real_idx in enumerate(true_indices):
                ...
                self.parse_action(real_idx, text)
    ...
    def parse_action(self, idx: int, text: str):
        """
