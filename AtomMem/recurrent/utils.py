...
class IncrementalChatTemplate:
    """
    增量式聊天模板：
    start()   -> 系统 + 首轮 user
    append()  -> 追加一条 user / assistant
    tokens()  -> 返回当前完整 token id
    """
    def __init__(self, tokenizer: PreTrainedTokenizer, max_sliding_window_size: int = 8192, system: str = None):
        self.max_len = max_sliding_window_size
        self.messages: List[Dict[str, str]] = []
        self.total_tokens = 0
        self._cached_lens: List[int] = []
        if system:
            self.messages.append({"role": "system", "content": system})
            self._cached_lens.append(
                len(tokenizer.apply_chat_template([self.messages[-1]], tokenize=True))
            )
            self.total_tokens += self._cached_lens[-1]

    def append(self, tokenizer: PreTrainedTokenizer, role: str, content: str) -> torch.LongTensor:
        """追加一条 user / assistant"""
        assert role in {"user", "assistant"}, "role must be 'user' or 'assistant'"
        new_msg = {"role": role, "content": content}
        self.messages.append(new_msg)
        new_len = len(tokenizer.apply_chat_template([new_msg], tokenize=True))
        self._cached_lens.append(new_len)
        self.total_tokens += new_len
        sys_idx = 1 if (self.messages and self.messages[0]["role"] == "system") else 0
        while self.total_tokens > self.max_len and len(self.messages) > sys_idx:
            oldest_len = self._cached_lens.pop(sys_idx)   # 删第一条 user/assistant
            del self.messages[sys_idx]
            self.total_tokens -= oldest_len

    def build(self, tokenizer: PreTrainedTokenizer) -> torch.LongTensor:
        """把当前 messages 转成 token id"""
        ids = tokenizer.apply_chat_template(
            self.messages,
            tokenize=True,
            add_generation_prompt=True,  # 自动加 <assistant>
            return_tensors="pt"
        ).squeeze(0)  # [L]
        return ids
...
def chat_template(tokenizer: PreTrainedTokenizer, system=False) -> str:
    if system:
