    ...
    def run_llm_loop(self, gen_batch, timing_raw) -> Tuple[DataProto, torch.BoolTensor, torch.LongTensor]:
        ...
        while not self.agent.done():
            ...
            with _timer('mt_update', timing_raw):
                ...
                # print(gen_output.meta_info["action_rate_metric"])
                if "action_rate_metric" in gen_output.meta_info:
                    for key, val in gen_output.meta_info["action_rate_metric"].items():
                        if key == "average_DB_size":
                            action_rate_metric[key] = val
                        else:
                            if key not in action_rate_metric:
                                action_rate_metric[key] = val
                            else:
                                action_rate_metric[key] += val
        ...
        dummy = DataProto()
        dummy = self.actor_rollout_wg.sleep(dummy)
        final_mask, sample_index, reward = self.agent.end()
        # convert the total number to rate
        if "action_rate_metric" in gen_output.meta_info:
            total = action_rate_metric.pop('total')
            for key, val in action_rate_metric.items():
                if key == "average_DB_size":
                    pass
                else:
                    action_rate_metric[key] = val/total
        ...
        if "action_rate_metric" in gen_output.meta_info:
            concat_gen_output.meta_info["metrics"] = action_rate_metric
