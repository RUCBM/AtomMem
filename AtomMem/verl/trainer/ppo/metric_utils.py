...
def process_validation_metrics(data_sources: list[str], sample_inputs: list[str], infos_dict: dict[str, list[Any]], seed: int = 42) -> dict[str, dict[str, dict[str, float]]]:
    ...
    # Not group metrics by prompt, cause we onlu need pass@1
    metric_set = {"mean", "std"}
    data_src2var2metric2vals = defaultdict(lambda: defaultdict(lambda: defaultdict(float)))
    for data_source in np.unique(data_sources):
        var2metric2vals = data_src2var2metric2vals[data_source]
        for var_name, var_vals in infos_dict.items():
            print("total_sample_number:",len(var_vals))
            var2metric2vals[var_name]["mean"] = np.mean(var_vals)
            var2metric2vals[var_name]["std"] = np.std(var_vals)
    return data_src2var2metric2vals
