    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if not self.config.recurrent.enable:
                ...
                if self.config.reward_model.use_custom_reward:
                    output_gen_batch.batch["final_mask"] = final_mask
                    output_gen_batch.batch["sample_index"] = sample_index
                    test_output_gen_batch = output_gen_batch.union(test_batch[sample_index])
                else:
            ...
            if not self.config.recurrent.enable:
                ...
                ###### ADD
                sample_inputs.extend(test_output_gen_batch.non_tensor_batch['sample_uuid'])
            ...
            reward_tensor, score_tensor, reward_extra_infos = compute_reward(test_output_gen_batch, self.val_reward_fn)

            rewards = reward_tensor.sum(-1).cpu().tolist()
            scores = score_tensor.sum(-1).cpu().tolist()
            sample_scores.extend(scores)
            print("rewards:", rewards)
            print("length of rewards:", len(rewards))
            reward_extra_infos_dict["score"].extend(scores)
            reward_extra_infos_dict["reward"].extend(rewards)

            data_source_lst.append(test_output_gen_batch.non_tensor_batch.get("data_source", ["unknown"] * reward_tensor.shape[0]))
        ...
        data_src2var2metric2vals = process_validation_metrics(data_sources, sample_inputs, reward_extra_infos_dict)
        metric_dict = {}
        for data_source, var2metric2vals in data_src2var2metric2vals.items():
            for var_name, metric2vals in var2metric2vals.items():
                for metric, val in metric2vals.items():
                    pfx = f"val/{data_source}/{var_name}/{metric}"
                    metric_dict[pfx] = val
        return metric_dict
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # with _timer('step', timing_raw):
                            ...
                            try:
                                workflow_metrics = gen_batch_output.meta_info["metrics"]
                            except:
                                workflow_metrics = {}
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        if not self.config.recurrent.enable:
                            if self.config.reward_model.launch_reward_fn_async:
                                ...
                                reward_tensor, score_tensor, reward_extra_infos_dict = compute_reward(batch, self.reward_fn)
                        else:
                            if self.config.reward_model.use_custom_reward:
                                reward_batch = batch.union(original_batch[sample_index])
                                reward_tensor, score_tensor, reward_extra_infos_dict = compute_reward(reward_batch, self.reward_fn)
                            else:
                                from recurrent.utils import final_batch
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        if not self.config.recurrent.enable:
                            ...
                            batch.batch['token_level_scores'] = score_tensor[sample_index]
                            batch.batch['token_level_rewards'] = reward_tensor[sample_index]
