...
from verl.utils.reward_score import default_compute_score, default_compute_score_LLM
import os
import numpy as np
...
class ThreadAgentRewardManager:
    """The reward manager.
    """
    actors = None
    def __init__(
        ...
        if reward_kwargs.get("use_llm", False):
            self.compute_score = compute_score or default_compute_score_LLM
        else:
    ...
    def __call__(self, data: DataProto, return_dict: bool = False):
        ...
        try:
            sample_index = data.batch["sample_index"]
            final_mask = data.batch["final_mask"]
        except:
            raise "There doesn't exists the key called 'sample_index' or 'final_mask' in the data."
        W = len(np.unique(sample_index))
        H = data.batch['responses'].shape[1]
        reward_tensor = torch.zeros([W,H], dtype=torch.float32)
        score_tensor = torch.zeros([W,H], dtype=torch.float32)
        sample_indices = defaultdict(list)
        for idx, value in enumerate(sample_index):
            sample_indices[value.item()].append(idx)

        args = []
        for index, indices in sample_indices.items():
            conversation = []
            final_conversation = []
            for indice in indices:
                data_item = data[indice]
                ...
                if not final_mask[indice]:
                    conversation.append([prompt_str, response_str])
                else:
                    final_conversation = [prompt_str, response_str]
            if "reward" in data.meta_info:
                extra_info = {"reward": data.meta_info["reward"][index]}
            else:
                extra_info = {}
            args.append([index,
                dict(data_source = data[indices[0]].non_tensor_batch['data_source'],
        ...
        for (i, result), valid_response_length in zip(results, valid_response_lengths):
            ...
            if isinstance(result, dict):
                reward = result.pop("reward") # for GRPO, do not want duplicate score/reward
                # Store the information including original reward
                reward_tensor[i, valid_response_length - 1] = reward
                score = result.pop("score")
                score_tensor[i, valid_response_length - 1] = score
            else:
                score = result
